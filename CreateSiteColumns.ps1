...
#
# From: http://blogs.captechconsulting.com/blog/kelly-rusk/bulk-creation-sharepoint-site-columns-and-content-types-powershell-part-1
#
# Description: 
#
# This script creates Site Columns in the appropriate Site
# Collections as listed in the CreateSiteColumns.csv. 
# The .CSV needs to be saved to "e:\sp\utilities\CreateSiteColumns.csv" directory.
...
$create = Import-Csv -path e:\sp\utilities\CreateSiteColumns.csv
ForEach($row in $create) {
 
# Get Site and Web object
$site = Get-SPSite -Identity $("http://" + $row.Department + ".roh.org.uk") 
$web = $site.RootWeb
...
$fieldXML = '<Field Type="'+$row.FieldType+'"
 ...
 Group="'+'_'+$row.Department+'_'+$row.Group+'"
 Hidden="'+$row.Hidden+'"
